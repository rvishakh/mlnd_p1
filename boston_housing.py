...
def explore_city_data(city_data):
    ...
    # Number of features?
    number_of_features = np.size(city_data.feature_names)
    ...
    std_deviation = np.std(housing_prices)


    print "Size of data (number of houses): " + str(number_of_houses)
...
def performance_metric(label, prediction):
    ...
    # Decision Tree Regressor uses MSE to measure the quality of the split
...
def split_data(city_data):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
				city_data.data, city_data.target, test_size=0.3, random_state=10)
